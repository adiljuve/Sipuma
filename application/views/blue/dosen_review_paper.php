				...
				<?php if($paper_detail_review): ?>	
					...
					<div class="block">
						...
						<?php $review_paper_count = $this->Sipuma_model->review_paper_count($paper_detail_review->paper_id, $this->session->userdata('user_id')); ?>
						<?php $revision_date = strtotime($paper_detail_review->latest_revision); ?>
						<?php $review_date_sql = $this->Sipuma_model->max_comment_review($paper_detail_review->paper_id, $this->session->userdata('user_id')); ?>
						<?php $latest_request = $this->Sipuma_model->latest_request($paper_detail_review->paper_id); ?>
						<?php if($latest_request):?>
						<?php $latest_request_date = strtotime($latest_request->review_date); ?>
						<?php endif; ?>
						<?php if($review_date_sql): ?>
						<?php $review_date = strtotime($review_date_sql->review_date); ?>
						<?php endif; ?>
						<?php //if((($paper_detail_review->paper_status == 0) && ($review_paper_count < 1)) || (($paper_detail_review->paper_status == 0) && ($revision_date >= $review_date))):?>
						<?php if(($paper_detail_review->paper_status == 0) && ($review_paper_count < 1)): ?>
						...
						<?php if($revision_date >= $latest_request_date): ?>
						| <a href="#revision" name="modal">[Merevisi]</a></p>
						<?php endif; ?>
						<?php else: ?>
						<h5><font color="red">Anda sudah melakukan review</font></h5>
						<?php endif; ?>
						<?php //if($paper_detail_review->revision_count >= 1): ?>
						<?php //kalo udah ada revisinya berarti ndak pake review_paper_count tapi revision_date > max_review_date buat hindari double review setelah revisi ?>
						<?php //endif; ?>
					</div>
					<div id="boxes">
