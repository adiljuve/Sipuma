				<div class="block">
					...
					<div class="block">
						...
						<?php if($paper_detail->file_name != ''): ?>
							<li><a href="<?php echo base_url().'mahasiswa/file/'.$paper_detail->paper_id; ?>" onclick="$(this).modal({width:900, height:500}).open(); return false;"><?php echo $paper_detail->file_name; ?></a></li>
						<?php else: ?>
							<li>-</li>
						<?php endif; ?>
						</ul>
					...
					<div class="block">
						<?php foreach($discussion_list as $discussion): ?>
							...
							<?php if ($paper_comment_form): ?>
							<div>
							<?php if($this->session->userdata('user_id') == $discussion->user_id): ?><a href="#" onClick="confirmation('<?php echo base_url().'mahasiswa/comment_delete/'.$discussion->discussion_id;?>')">[Hapus]</a> | <?php endif; ?><a href="#reply<?php echo $discussion->discussion_id; ?>" id="<?php echo $discussion->discussion_id; ?>" class="title" name="<?php echo $discussion->discussion_id; ?>">[Balas Komentar]</a>
							</div>
							...
							<form name="discussion_<?php echo $discussion->discussion_id; ?>" id="discussion_<?php echo $discussion->discussion_id; ?>" action="<?php echo base_url().'mahasiswa/comment_reply'; ?>" method="post" onSubmit="return discussion_reply_check(discussion_<?php echo $discussion->discussion_id; ?>)">
								<fieldset>
									<p>
							...
							<?php foreach($discussion_list_child as $child): ?><?php $user_detail = $this->Sipuma_model->user_detail($child->user_id); ?>
								...
								<?php if($this->session->userdata('user_id') == $child->user_id): ?><a href="#" onClick="confirmation('<?php echo base_url().'mahasiswa/comment_delete/'.$child->discussion_id;?>')">[Hapus]</a><?php endif; ?>
								<hr />
							<?php endforeach; ?>
							</div>
							<?php endif; ?>
