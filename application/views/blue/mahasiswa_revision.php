			<div class="block">
				<?php $paper_review = $this->Sipuma_model->paper_review($paper_rejected->paper_id); ?>
				<?php if($paper_review): ?>	
				<h5>Review</h5>
				<div class="first article">
					<?php foreach($paper_review as $review): ?>							
						...
						<p><b>Hasil Review: Publikasi</b></p>
						<?php endif; ?>
						<hr />
					<?php endforeach; ?>
				</div>
					...
					<form name="paper" id="paper" action="" method="post" enctype="multipart/form-data">
						<fieldset>
							...
							<p>
								...
								<input type="text" name="title" id="title" value="<?php echo $paper_rejected->title; ?>" />
							</p>
							<p class="both">
							...
							<p>
								...
								<textarea cols="43" rows="10" name="abstraction" id="abstraction"><?php echo $paper_rejected->abstraction; ?></textarea>
							</p>
							<p>
								<label>Ubah File Lampiran(.pdf): (*)</label>
								<?php echo form_error('file','<br /><label><font color="red"><b>','</b></font></label>'); ?>
								<input type="file" name="file" id="file" />
								<?php if($paper_rejected->file_name != ''): ?>
									<img src="<?php echo base_url().'style/img/pdf_document.png'; ?>" /> <a href="<?php echo base_url().'mahasiswa/file/'.$paper_rejected->paper_id; ?>" onclick="$(this).modal({width:900, height:500}).open(); return false;"><?php echo $paper_rejected->file_name; ?></a>
								<?php endif; ?>
									<input type="hidden" name="file_exist" id="file_exist" value="<?php echo $paper_rejected->file_name; ?>" />
							</p>
							<h5>File Lama:</h5>
							<?php $file_revision = $this->Sipuma_model->file_revision($paper_rejected->paper_id); ?>
							<ul class="file">
